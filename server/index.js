...
app.post('/api/verify', async (req, res) => {
    ...
    try {
        ...
        if(verified) {
            const user = await User.findOne({_id: verified._id}).select('-password')
            res.status(200).json({
...
app.post('/api/updatePassword', async (req, res) => {
    ...
    try {
        ...
        bcrypt.hash(password, 13, async function(err, hash) {
            // Store hash in your password DB.
        const update = await User.findOneAndUpdate({_id: decoded._id}, {password: hash})
        res.status(200).json({
...
app.post('/api/updateInterest', async (req, res) => {
    ...
    try {
        const decoded = await jwt.verify(token, process.env.JWT_SECRET)
        const update = await User.findOneAndUpdate({_id: decoded._id}, {interests: interest})
        res.status(200).json({
